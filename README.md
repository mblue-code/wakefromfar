...
Community und Projektrichtlinien:
...
- Historische Planungs- und Sprintdokumente liegen unter `docs/archive/`.
...
- Docker Compose erwartet eine lokale `.env`-Datei im Repo-Root. Starte jeden Compose-Flow erst nach `cp .env.example .env`.
...
cp .env.example .env
# .env Werte setzen (APP_SECRET, ADMIN_PASS)
# RATE_LIMIT_BACKEND kann auf memory bleiben; die Redis-Overlay-Datei setzt redis explizit.
...
- Default/local Docker uses `RATE_LIMIT_BACKEND=memory` for single-instance startup without Redis.
...
- This repo uses `network_mode: host` for the backend because that is the most reliable path for WoL/LAN traffic on a real host.
- On a normal Linux Docker Engine install, that host-network setup is the native path and should expose the backend on the Linux host directly.
- On macOS, Docker Desktop runs Linux containers inside a VM. Host networking is supported only on Docker Desktop `4.34+` and must be enabled explicitly in Docker Desktop settings.
- If you are on macOS and `docker compose up -d --build` succeeds but `curl http://127.0.0.1:8080/health` still returns `Connection refused`, check Docker Desktop first:
  - sign in to Docker Desktop
  - open `Settings`
  - go to `Resources -> Network`
  - enable `Enable host networking`
  - apply and restart Docker Desktop
- Docker documents that Docker Desktop host networking is layer-4 only. For this repo that is usually acceptable for HTTP/TCP/UDP checks, but Linux remains the more trustworthy environment for real WoL/network validation.
- If you only want to inspect the API or Admin UI locally on a Mac, the better supported workaround is a Mac-specific compose override that publishes `127.0.0.1:8080:8080` instead of relying on host networking.
- If you want the closest-to-production networking behavior for Wake-on-LAN, prefer a normal Linux host or run the backend directly on the machine that will actually send the packets.
...
- Wenn trotz Proxy-Trust `{"detail":"Client IP not allowed"}` auftritt und Logs nur eine Proxy-IP (z.B. `172.18.0.1`) zeigen, wird die echte Client-IP upstream nicht mehr weitergereicht. In diesem Fall Netzwerkfilterung am Edge-Proxy erzwingen und die Proxy-IP/CIDR als Fallback in `IP_ALLOWLIST_CIDRS` sowie `ADMIN_IP_ALLOWLIST_CIDRS` aufnehmen.
...
- Invite-Onboarding ist im Backend derzeit deaktiviert; Android enthält noch Legacy-UI/Codepfade dafür, aber sie gehören nicht zur aktuell unterstützten Produktoberfläche.
