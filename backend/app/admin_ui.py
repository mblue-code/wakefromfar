...
def _validate_admin_ui_post_origin(request: Request) -> None:
    ...
    if request.url.path == "/admin/ui/login" and not referer and (not origin or origin.lower() == "null"):
        _security_ui_event(
            request,
            counter="security.admin_ui.origin_missing_allowed",
            event="security.admin_ui.origin_missing_allowed",
            reason="login_origin_null_and_referer_missing",
            origin=origin or None,
        )
        return

    reason = "referer_mismatch" if not origin else "origin_null_referer_mismatch"
    _security_ui_event(
        request,
        counter="security.admin_ui.origin_failed",
        event="security.admin_ui.origin_failed",
