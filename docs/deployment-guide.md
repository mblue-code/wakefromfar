...
- the canonical local reset flow lives in `docs/local-reset-workflow.md`
...
- Sprint-8 pre-production verification is documented in `docs/security-preproduction-checklist.md`
...
All Docker Compose commands in this repo expect that local repo-root `.env` file to exist first.
...
Reverse-proxy troubleshooting (`{"detail":"Client IP not allowed"}`):

- Symptom: backend returns `403` with `{"detail":"Client IP not allowed"}` for proxied requests, even though your users should match the configured allowlist.
- Check what the backend actually sees:
  - `docker compose logs -f wol-backend | grep -E "security.ip_allowlist.blocked|security.admin_network.blocked"`
  - If blocked events show a proxy IP (for example `172.18.0.1`) instead of a real client IP (`100.x` for Tailscale), backend-side allowlisting is evaluating the wrong source address.
- Confirm proxy trust wiring:
  - `TRUST_PROXY_HEADERS=true`
  - `TRUSTED_PROXY_CIDRS` contains the real source network that reaches backend (often Docker bridge CIDRs, not only loopback).
  - Proxy forwards `X-Forwarded-For` and `X-Forwarded-Proto`.
- If your upstream chain still collapses all traffic to a single proxy IP:
  - Keep strict client filtering at the edge proxy/load-balancer (Traefik `ipAllowList`, Cloudflare/Tailscale policy, firewall/ACLs).
  - Add only that proxy source IP/CIDR to backend `IP_ALLOWLIST_CIDRS` and `ADMIN_IP_ALLOWLIST_CIDRS` as a compatibility fallback.
  - Example fallback: append `172.18.0.1/32` when backend logs show only that source.
...
- Default `RATE_LIMIT_BACKEND=memory` keeps single-instance/local deployments bootstrappable without Redis.
- `RATE_LIMIT_BACKEND=redis` is the upgrade path for durable/shared limits across multiple backend instances.
